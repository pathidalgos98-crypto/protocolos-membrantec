...
def _try_num(value):
    """Si el value es string y luce a número, lo convierte a float/int para que
    las formulas de Excel (=V22*U22) puedan recalcularlo."""
...
def generar_ri(patch, proyecto, registros):
    ...
    for r in registros:
        ...
        patch.set(f"P{fila}", r.get("espesor"))      # mm/gr/m2
        patch.set(f"Q{fila}", r.get("st_largo"))     # Largo ST
        patch.set(f"R{fila}", r.get("st_ancho"))     # Ancho ST
        patch.set(f"S{fila}", r.get("st_m2"))        # M2 ST
        patch.set(f"U{fila}", r.get("ct_largo"))     # Largo CT
        patch.set(f"V{fila}", r.get("ct_ancho"))     # Ancho CT
        patch.set(f"W{fila}", r.get("ct_m2"))        # M2 CT
